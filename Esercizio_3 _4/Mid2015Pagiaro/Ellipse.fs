...
open Tratti
open ap
type Ellipse() as this =
    inherit Tratti()
    (*ESTREMAMENTE SIMILE, PRATICAMENTE UGUALE SE NON PER ONPAINT, a RECTANGLE*)
    ...
    override this.OnMouseMove e =
        ...
        if this.Selected then
            ...
            | 0 ->
                startPoint <- pt
                this.Size <- SizeF(Math.Abs(this.Size.Width - (pt.X - leftTop.X)),
                                     Math.Abs(this.Size.Height - (pt.Y - leftTop.Y)))
            ...
            | 1 -> //ROTATE 
                ...
                //updateManigliePosition leftTop this.Size.this.Size.Width this.Size.this.Size.Height
            | 3 -> // TRANSLATE
            ...
            | _ -> ()
        else //Codice eseguito alla creazione
            endPoint <- PointF(single e.Location.X, single e.Location.Y)
            this.Size <- SizeF(Math.Abs(startPoint.X - endPoint.X), Math.Abs(startPoint.Y - endPoint.Y))
            leftTop <- PointF(Math.Min(startPoint.X, endPoint.X), Math.Min(startPoint.Y, endPoint.Y))
    ...
    override this.OnPaint (e:PaintEventArgs) =
        ...
        if this.Selected then
            ...
            e.Graphics.DrawRectangle(p, rect.X, rect.Y, this.Size.Width, this.Size.Height)
